    ...
    def parse_stats(self, stats):
        ...
        for pod in stats["pods"]:
            ...
            if "availableBytes" in pod["memory"]:
                m.add_value("memory_available", pod["memory"]["availableBytes"])
            if "usageBytes" in pod["memory"]:
                m.add_value("memory_usage", pod["memory"]["usageBytes"])
            if "workingSetBytes" in pod["memory"]:
                m.add_value("memory_working_set", pod["memory"]["workingSetBytes"])
            ...
            for container in pod["containers"]:
                ...
                m.add_value("cpu_usage", container["cpu"]["usageNanoCores"]/1000000000)
                if "availableBytes" in container["memory"]:
                    m.add_value("memory_available", container["memory"]["availableBytes"])
                if "usageBytes" in container["memory"]:
                    m.add_value("memory_usage", container["memory"]["usageBytes"])
                if "workingSetBytes" in container["memory"]:
                    m.add_value("memory_working_set", container["memory"]["workingSetBytes"])
